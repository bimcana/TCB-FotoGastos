...
> Estado: **Fase 4 (cierre)** — carpeta matriz **compartida multi-usuario** sin pérdida de datos: cada factura viaja con sus metadatos en la `description` del archivo y el índice se **auto-repara** al listar (lo perdido se restaura; lo ajeno aparece como **«Sin procesar»** y se procesa con el pipeline completo — recorte automático/manual, datos IA·OCR — enviando el original a la papelera). Gastos ahora es un **acordeón por carpetas** (expandir/colapsar; Generar por mes). Permiso de Drive amplio (la app solo opera dentro de la carpeta matriz; requiere reconectar una vez). Regla de altura del PDF **calibrada con 57 facturas reales**: nada se divide salvo tickets de supermercado (ratio > 4). Preparada para la futura versión **Lite** (alimentador de carpeta). Sobre la **Fase 3** — botón **«Generar documento de Gastos»** en Gastos: PDF réplica de la plantilla de la firma (portada con logo, membrete por página, 3 facturas por página con RD$, tickets largos en 2 columnas, © TCB al pie) + **Excel Formato 606** básico (tipo de bienes/servicios lo completa la firma), ambos generados EN el teléfono (pdf-lib + SheetJS vendorizados), subidos a la carpeta del mes en Drive y compartidos con la hoja de iOS. Nuevo **perfil de Empresa** en Ajustes (membrete, replicado como `_empresa.json`) y **selector de mes** en Gastos. Sobre la **Fase 2E** — autorecorte de importación con **IA local** (U²-Net-p sobre ONNX Runtime WASM, ~18 MB de descarga única cacheada por el service worker; funciona offline): el editor de esquinas abre siempre con el documento ya detectado, estilo Adobe Scan. Revisor con Gemini **visible**: arranca solo tras cada subida, chip animado "Leyendo con IA…" en Gastos, el panel abierto se rellena al terminar, y botón **"Leer con IA"** como reintento manual. **Reconexión automática de Drive** al abrir (o aviso tocable "Reconectar" en Gastos). El icono de cola abre un **panel** para ver/reintentar/eliminar lo que espera conexión. La cámara en vivo vuelve al criterio estricto (sin falsos positivos sobre fondos texturados). Sobre la **Fase 2D** (guardado provisional + re-archivado automático + toggle IA/OCR + esquinas con lupa).
>
